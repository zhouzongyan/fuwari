...
const TwikooFn = async (commentDOM: string) => {
  ...
  twikoo.init({ envId: siteConfig.Comment.Twikoo.envId, el: commentDOM, onCommentLoaded: () => setTimeout(() => document.querySelectorAll('.vh-comment a[href="#"]').forEach(link => link.removeAttribute('href'))) })
}
...
const WalineFn = async (commentDOM: string, walineInit: any) => {
  ...
  walineInit = init({
    el: commentDOM, path: window.location.pathname.replace(/\/$/, ''), serverURL: siteConfig.Comment.Waline.serverURL,
...
// 检查是否开启评论
const checkComment = () => {
  const CommentARR: any = Object.keys(siteConfig.Comment);
  const CommentItem = CommentARR.find((i: keyof typeof siteConfig.Comment) => siteConfig.Comment[i].enable);
  return CommentItem;
}
